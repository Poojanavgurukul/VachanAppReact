...
const MainProvider = (props) => {
  const { language, downloaded, sourceId, baseAPI, parallelLanguage } = props;
  const [bookList, setBookList] = useState([]);
  const [parallelBookList, setParallelBookList] = useState([]);
  ...
  const getParallelBookList = async () => {
    try {
      let bookListData = [];
      if (downloaded) {
        let response = await DbQueries.getDownloadedBook(parallelLanguage);
        if (response != null) {
          for (var i = 0; i <= response.length - 1; i++) {
            let books = {
              bookId: response[i].bookId,
      ...
      setParallelBookList([]);
      console.log("ERROR parall ", error);
    }
  };
...
const mapStateToProps = (state) => {
  return {
    ...
    parallelLanguage: state.selectContent.parallelLanguage.languageName,
