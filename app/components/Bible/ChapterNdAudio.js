...
const ChapterNdAudio = (props) => {
  ...
  return (
    ...
    >
      {(currentVisibleChapter == 1 && props.downloaded == true) ||
      (previousContent &&
        Object.keys(previousContent).length == 0 &&
        previousContent.constructor === Object) ? null : (
        <View
          style={[
            styles.bottomBarPrevView,
            showBottomBar ? styles.showBottomBar : styles.hideBottomBar,
            props.visibleParallelView
              ? styles.bottomBarParallelPrevView
        ...
        >
          <Icon
            ...
            size={props.visibleParallelView ? 16 : 32}
            style={styles.bottomBarChevrontIcon}
            onPress={() =>
              queryBookFromAPI(props.downloaded ? false : previousContent)
            }
          />
        </View>
      )}
      {audio && status && (
        ...
        >
          <Player styles={styles} chapter={currentVisibleChapter} />
        </View>
      )}
      {(currentVisibleChapter == props.totalChapters && props.downloaded) ||
      (nextContent &&
        Object.keys(nextContent).length == 0 &&
        nextContent.constructor === Object) ? null : (
        <View
          style={[
            styles.bottomBarNextView,
            showBottomBar ? styles.showBottomBar : styles.hideBottomBar,
            props.visibleParallelView
              ? styles.bottomBarNextParallelView
              : styles.bottomBarPosition,
          ]}
        >
          <Icon
            ...
            style={styles.bottomBarChevrontIcon}
            onPress={() =>
              queryBookFromAPI(props.downloaded ? true : nextContent)
            }
          />
        </View>
      )}
